...
- [x] Review `server/server.properties` and replace the default `motd`, player limit, difficulty and gameplay settings. Customized: `motd=Jarock Minecraft 26.2 Server`, `max-players=20`, `difficulty=normal`, `gamemode=survival`. The `online-mode` rewrite preserves the `motd` byte-for-byte (Latin-1/UTF-8/BOM safe).
