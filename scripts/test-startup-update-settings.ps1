...
try {
    ...
    Assert ($ParameterManager.Contains('C. Clean Jarock cache')) 'parameter-manager.bat exposes the cache-cleanup option'
    Assert ($ParameterManager.Contains('choice /c 1234567890XYIEUC')) 'parameter-manager.bat includes C in the menu choices'
    Assert ($ParameterManager.Contains('if errorlevel 16 goto clean_cache_menu')) 'parameter-manager.bat maps C to the cache-cleanup branch'
    Assert ($ParameterManager.Contains('call "%ROOT%\scripts\classic-console.bat" "Jarock cache cleanup" "%ROOT%\clean-cache.bat" /wait')) 'parameter-manager.bat opens cache cleanup in a separate classic console window'
    Assert (Test-Path -LiteralPath (Join-Path $Root 'clean-cache.bat') -PathType Leaf) 'the cache cleanup batch entry point is present'
    Assert (Test-Path -LiteralPath (Join-Path $Root 'scripts/clean-cache.ps1') -PathType Leaf) 'the cache cleanup PowerShell implementation is present'
    ...
    Assert ($TuiEntry.Contains('enableMouseMovement: true')) 'TUI enables mouse movement tracking for hover selection'
    Assert ($TuiEntry.Contains('autoFocus: false')) 'TUI keeps focus under explicit menu control after renderer startup'
    ...
    Assert ($TuiEntry.Contains('key.__jarockHandled')) 'TUI marks handled key events to prevent duplicate actions'
    Assert ($TuiEntry.Contains('renderer.keyInput.on("keypress"')) 'TUI has a guarded renderer-level Enter fallback'
    Assert ($TuiEntry.Contains('activeActionSelect')) 'TUI tracks the active Select for input fallback'
    Assert ($TuiEntry.Contains('function focusActionSelect')) 'TUI centralizes active Select registration and focus'
    Assert ($TuiEntry.Contains('focusActionSelect(menu)')) 'TUI restores the main menu as the active Select'
    Assert ($TuiEntry.Contains('main.add(settingsMenu)')) 'TUI attaches the parameter settings menu to the render tree'
    Assert ($TuiEntry.Contains('focusActionSelect(settingsMenu)')) 'TUI focuses the parameter settings menu'
    Assert ($TuiEntry.Contains('focusActionSelect(menu)')) 'TUI focuses the main menu after starting the renderer'
    Assert ($TuiEntry.Contains('function createActionSelect')) 'TUI gives every Select a direct activation callback'
    Assert ($TuiEntry.Contains('select.__jarockActivate = activate')) 'TUI stores the action callback for mouse activation'
    Assert ($TuiEntry.Contains('function selectIndexAt')) 'TUI converts mouse coordinates into Select option indexes'
    Assert ($TuiEntry.Contains('function selectMouseMove')) 'TUI follows the mouse over visible Select options'
    Assert ($TuiEntry.Contains('function selectMouseDown')) 'TUI defines an explicit Select mouse handler'
    Assert ($TuiEntry.Contains('function selectMouseUp')) 'TUI defines a mouse-up fallback for Windows console hosts'
    Assert ($TuiEntry.Contains('function getSelectedActionOption')) 'TUI resolves the selected action from Select state'
    Assert ($TuiEntry.Contains('function invokeAction')) 'TUI reports action callback failures instead of swallowing them'
    Assert ($TuiEntry.Contains('function activateBoundSelect')) 'TUI has a defined click activation helper'
    Assert ($TuiEntry.Contains('const offset = Math.max(0')) 'TUI mouse handler accounts for Select scrolling'
    Assert ($TuiEntry.Contains('onMouseMove: (event: any) => selectMouseMove.call(select, event)')) 'TUI binds mouse hover to the concrete Select instance'
    Assert ($TuiEntry.Contains('onMouseDown: (event: any) => selectMouseDown(select, event)')) 'TUI binds mouse activation to the concrete Select instance'
    Assert ($TuiEntry.Contains('onMouseUp: (event: any) => selectMouseUp(select, event)')) 'TUI binds mouse-up fallback to the concrete Select instance'
    Assert ($TuiEntry.Contains('onKeyDown: (key: any) => selectKeyDown(select, key)')) 'TUI binds Enter to the concrete Select instance'
    Assert ($TuiEntry.Contains('{ name: "Clean .cache"')) 'TUI exposes the cache-cleanup action'
    Assert ($TuiEntry.Contains('case "cache": runClassicConsoleBatch(entry("clean-cache.bat"))')) 'TUI routes cache cleanup through the shared CLI launcher'
    Assert ($TuiEntry.Contains('activateBoundSelect(select)')) 'TUI mouse and Enter handlers execute the concrete selected action'
    Assert ($TuiEntry.Contains('spawn("cmd.exe", ["/d", "/c", "call", helper, "Jarock operation", wrapper, "/wait"]')) 'TUI launches batch actions with Windows-safe argument quoting'
    ...
    Assert (-not $AutoRelease.Contains('LEGACY_FULL_NAME')) 'auto-release no longer generates the obsolete jarock-full compatibility alias'
    Assert (-not $AutoRelease.Contains('LEGACY_LITE_NAME')) 'auto-release no longer generates the obsolete jarock-lite compatibility alias'
    Assert (-not $AutoRelease.Contains('legacy_full_name')) 'auto-release no longer uploads obsolete legacy package outputs'
    Assert (-not $AutoRelease.Contains('legacy_lite_name')) 'auto-release no longer uploads obsolete legacy package outputs'
