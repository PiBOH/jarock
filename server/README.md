...
The repository stores templates, the loader marker reference, loader-specific pinned manifests, the tracked runtime icon at `server/icon.png` and the tracked multiplayer server-list icon at `server/server-icon.png`. Minecraft uses `server/server-icon.png` for the icon shown in the multiplayer server list. Both server-scoped icons are included in the automatic Full and Lite release packages. `server/server.jar` is generated locally for the selected loader and remains ignored by Git. Fabric also retains the local vanilla engine as `server/vanilla-server.jar`; NeoForge uses its official generated `run.bat` and libraries.
