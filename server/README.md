...
The repository stores templates, the loader marker reference, loader-specific pinned manifests, the tracked runtime icon at `server/icon.png` and the tracked multiplayer server-list icon at `server/server-icon.png`. Minecraft uses `server/server-icon.png` for the icon shown in the multiplayer server list. Both server-scoped icons are included in the automatic Full and Lite release packages and are preserved by `clean-server-runtime.bat`; an optional `server/logo.png` is preserved too if present. The repository-root `icon.png` and `logo.png` are outside the cleanup scope and are also preserved. `server/server.jar` is generated locally for the selected loader and remains ignored by Git. Fabric also retains the local vanilla engine as `server/vanilla-server.jar`; NeoForge uses its official generated `run.bat` and libraries.
...
6. Before the first Minecraft process starts, Jarock applies `server/config/welcomemessage.json5.template-jarock` as the configured `welcomemessage.json5`. This one-time migration replaces the mod's generic first-run file; later starts preserve operator edits. The template is retained by `clean-server-runtime.bat` and is refreshed by a verified Lite-package update.
