...
3. The bootstrap installs the selected loader and downloads only its matching pinned server mods. Fabric also installs server-side Links In Chat for clickable chat URLs and the `/link`/`/linkwhisper` commands, plus Welcome AWA for configurable colored join messages; clients do not need to install these mods. It also verifies Better Multiplayer Sleep and places it in the configured world's `datapacks/` folder for both Fabric and NeoForge.
