...
- Choose `U. Check for Jarock updates` in `parameter-manager.bat` to check GitHub without starting the server. If a verified compatible package for the installed CLI/TUI and Full/Lite edition is available, the updater asks `Download and install it now? (y/N)`; Enter or `N` leaves the installation unchanged.
