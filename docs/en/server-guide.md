...
| Welcome Message | Configurable server-side welcome messages; requires Collective | `mods/`, Fabric or NeoForge 26.2; Jarock applies `config/welcomemessage.json5.jarock` once on first setup and preserves later edits |
...
### 14.1 World import and export

Jarock can import and export worlds from `parameter-manager.bat`.

**Import a world** (`I. Import world`): type the full path of a world folder (containing `level.dat`) or of a `.zip` world archive, or leave the input empty to open a Windows folder picker; type `CLEAR` to remove the request. On the next `start-server.bat` run, Jarock imports the world into the configured `level-name` folder (`server\world` by default) before the server starts. If a world already exists there, Jarock asks for confirmation and first moves it aside as `server\<name>_originalbkp` (with a timestamp suffix when that name is already taken), so no data is lost. The parameter manager then asks `Remember this world for future starts? (Y/n)`, with Yes as the default. If remembered, `WORLD_IMPORT_SOURCE` remains saved and is imported again at every start: Jarock asks for confirmation and first moves the existing world aside as `server\<name>_originalbkp` (with a timestamp suffix when that name is already taken), so the imported world replaces the active one after the operator confirms. An incomplete world folder (for example missing `level.dat` after a crash) is also replaced after confirmation, with a backup first. If you answer `n`, the request is cleared after the one-shot import. The imported world is a fresh copy: the source folder or archive is never modified.

**Export a world** (`E. Export world`): type the full path of a destination folder outside `server/`, or leave the input empty to open a Windows folder picker; type `CLEAR` to remove the request. After every clean shutdown (`stop` and the final `SAFE TO CLOSE` confirmation) Jarock mirrors the current world into that destination folder, overwriting it. The destination must stay outside `server/`: a world folder inside the server directory would be detected as possible previous world data on the next start. The export runs only after a clean shutdown, never after a crash or a forced close.
