...
Essential Commands 0.41.0 and its required `ec-core` 1.3.0 component are installed on Fabric for Minecraft 26.2 and provide useful server commands. No compatible NeoForge 26.2 build is available, so NeoForge does not install Essential Commands. No Chat Reports is installed server-side for both supported loaders. It prevents the server from forwarding signed chat-reporting data, but vanilla clients may still show an unsigned-chat warning unless they also use a compatible client-side setup. Jarock does not change `enforce-secure-profile` automatically; keep the server policy explicit and test the clients you use.
