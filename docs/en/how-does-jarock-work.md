...
9. Every pinned downloaded file is checked with SHA-512. DedicatedPower is the exception: it is a Fabric-only mod, updated automatically from the latest GitHub release, and its size and checksum are verified after download. Essential Commands 0.41.0 and its required `ec-core` 1.3.0 component are pinned to the reviewed Fabric 26.2 Modrinth version and checksums; no compatible NeoForge 26.2 build is available. Links In Chat, Welcome AWA and No Chat Reports are pinned to their reviewed Modrinth versions and checksums. Jarock also verifies Better Multiplayer Sleep 1.1.0 and installs it as a datapack in the configured world's `datapacks/` folder for Fabric and NeoForge without replacing existing worlds or datapacks.
...
The default Fabric stack contains Fabric API, Geyser-Fabric, Floodgate-Fabric, Lithium, FerriteCore, Krypton, ServerCore, Fabric Carpet, Essential Commands with `ec-core`, Links In Chat, Welcome AWA, No Chat Reports and DedicatedPower. Essential Commands adds useful server commands and is Fabric-only for Minecraft 26.2 because no compatible NeoForge build is available. The loader-neutral Better Multiplayer Sleep datapack is installed separately in the configured world's `datapacks/` folder; use `/reload` after manual datapack changes. Links In Chat is server-side and adds clickable URLs plus `/link` and `/linkwhisper` commands. Welcome AWA is server-side and sends configurable colored join messages using `%player%`; its `welcome reload` command reloads `config/welcome-mod.json`. Players do not need to install either mod on their clients. It does not install arbitrary Bukkit, Spigot or Paper plugins and it does not add client-only content such as Sodium to the server.
