...
9. Every pinned downloaded file is checked with SHA-512. DedicatedPower is the exception: it is a Fabric-only mod, updated automatically from the latest GitHub release, and its size and checksum are verified after download. Links In Chat and Welcome AWA are pinned to their reviewed Fabric 26.2 Modrinth versions and checksums. Jarock also verifies Better Multiplayer Sleep 1.1.0 and installs it as a datapack in the configured world's `datapacks/` folder for Fabric and NeoForge without replacing existing worlds or datapacks.
