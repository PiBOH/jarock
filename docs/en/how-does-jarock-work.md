...
After confirmation, it downloads the matching package for the installed edition (`jarock-cli-lite.zip`, `jarock-cli-lite-<version>.zip`, `jarock-tui-lite.zip` or `jarock-tui-lite-<version>.zip`, depending on the marker in `scripts/jarock-edition.ini`) and its published SHA-512 checksum. It verifies the archive before extraction, checks the package version and excludes `.github/` and `.website/`. Existing installations use the same interface and tier after updates: CLI stays CLI, TUI stays TUI, Full stays Full and Lite stays Lite. The Lite package is used for Lite installations because they already have their Java prerequisites; the updater does not download or reinstall them. It updates only project files from the matching package and preserves the generated `server/` directory, world data, mods, libraries, server properties, EULA, Geyser/Floodgate keys, local launch settings, Java selection, logs and updater cache. A rollback copy of overwritten project files is stored under `.cache/update-backups/`.
