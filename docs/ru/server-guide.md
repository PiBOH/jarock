...
> Technical fallback note: Links In Chat is included in the pinned Fabric 26.2 server stack. It makes URLs in server chat clickable and adds `/link` and `/linkwhisper`; clients do not need to install it. Welcome Message 2.8 with its required Collective library is included as a verified server-side Minecraft 26.2 mod for Fabric and NeoForge; it sends configurable join messages. On the first Jarock-managed startup, Jarock applies its configured `welcomemessage.json5` template once and preserves later operator edits. Если шаблон отсутствует (например, после обновления со старой версии), bootstrap автоматически восстанавливает его при запуске. Скрипт очистки сохраняет отслеживаемый шаблон, а проверенные обновления Lite-пакета обновляют его. Проверенные обновления Lite-пакета требуют шаблон и устанавливают его из пакета, даже если его нет среди установленных файлов сервера.
