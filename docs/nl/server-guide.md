...
Installeer 64-bits Java 25, start `start-server.bat` en gebruik `parameter-manager.bat` voor RAM en GUI of `nogui`. Lees `server/eula.txt` en zet pas na akkoord met de EULA `eula=true`. Gebruik Fabric, Geyser-Fabric en Floodgate-Fabric en maak backups. Jarock wijzigt router, firewall en port forwarding niet.
