...
> Technical fallback note: Links In Chat is included in the pinned Fabric 26.2 server stack. It makes URLs in server chat clickable and adds `/link` and `/linkwhisper`; clients do not need to install it. Welcome Message 2.8 with its required Collective library is included as a verified server-side Minecraft 26.2 mod for Fabric and NeoForge; it sends configurable join messages. On the first Jarock-managed startup, Jarock applies its configured `welcomemessage.json5` template once and preserves later operator edits. Dacă șablonul lipsește (de exemplu, după o actualizare de la o versiune mai veche), bootstrap-ul îl restaurează automat la pornire. Scriptul de curățare păstrează șablonul urmărit, iar actualizările verificate ale pachetului Lite îl reîmprospătează. Actualizările verificate ale pachetului Lite impun șablonul și îl instalează din pachet chiar și atunci când nu este prezent printre fișierele serverului instalat.
