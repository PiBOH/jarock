...
> Technical fallback note: Links In Chat is included in the pinned Fabric 26.2 server stack. It makes URLs in server chat clickable and adds `/link` and `/linkwhisper`; clients do not need to install it. Welcome Message 2.8 with its required Collective library is included as a verified server-side Minecraft 26.2 mod for Fabric and NeoForge; it sends configurable join messages. On the first Jarock-managed startup, Jarock applies its configured `welcomemessage.json5` template once and preserves later operator edits. Ako predložak nedostaje (npr. nakon ažuriranja sa starije verzije), bootstrap ga automatski vraća pri pokretanju. Skripta za čišćenje čuva praćeni predložak, a provjerena ažuriranja Lite paketa ga osvježavaju. Provjerena Lite ažuriranja paketa zahtijevaju predložak i instaliraju ga iz paketa čak i kada nije prisutan među instaliranim datotekama poslužitelja.
