...
- **Welcome Message:** mod server-side che invia messaggi configurabili quando entra un giocatore; richiede la libreria Collective ed è disponibile per Fabric e NeoForge 26.2. Al primo avvio gestito da Jarock viene applicato `server/config/welcomemessage.json5.template-jarock` come `server/config/welcomemessage.json5`; gli avvii successivi conservano le tue modifiche.
...
> **Icone Jarock:** il file root `icon.png` viene usato solo come icona predefinita per un mondo che non possiede già una propria icona; le icone di mondi importati o personalizzati vengono preservate. `server/server-icon.png` è l'icona mostrata nell'elenco multiplayer. Entrambi i file sono inclusi nei pacchetti di rilascio Full e Lite.
