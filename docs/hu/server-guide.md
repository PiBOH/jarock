...
> Technical fallback note: Links In Chat is included in the pinned Fabric 26.2 server stack. It makes URLs in server chat clickable and adds `/link` and `/linkwhisper`; clients do not need to install it. Welcome Message 2.8 with its required Collective library is included as a verified server-side Minecraft 26.2 mod for Fabric and NeoForge; it sends configurable join messages. On the first Jarock-managed startup, Jarock applies its configured `welcomemessage.json5` template once and preserves later operator edits. Ha a sablon hiányzik (például egy régebbi verzióról történő frissítés után), a bootstrap induláskor automatikusan visszaállítja. A tisztító parancsfájl megőrzi a verziókövetett sablont, a hitelesített Lite-csomagfrissítések pedig frissítik. Az ellenőrzött Lite-csomagfrissítések megkövetelik a sablont, és telepítik azt a csomagból akkor is, ha nincs jelen a telepített szerverfájlok között.
