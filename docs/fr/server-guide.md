...
> Technical fallback note: Links In Chat is included in the pinned Fabric 26.2 server stack. It makes URLs in server chat clickable and adds `/link` and `/linkwhisper`; clients do not need to install it. Welcome Message 2.8 with its required Collective library is included as a verified server-side Minecraft 26.2 mod for Fabric and NeoForge; it sends configurable join messages. On the first Jarock-managed startup, Jarock applies its configured `welcomemessage.json5` template once and preserves later operator edits. Si le modèle manque (par exemple après une mise à jour depuis une version antérieure), le bootstrap le restaure automatiquement au démarrage. Le script de nettoyage préserve le modèle suivi, et les mises à jour vérifiées du paquet Lite le renouvellent. Les mises à jour vérifiées du pack Lite exigent le modèle et l'installent depuis le pack même lorsqu'il n'est pas présent parmi les fichiers du serveur installé.
