...
> **Interface support status:** The CLI is **strongly recommended** and is the maintained, supported interface. The TUI remains available in future release packages, but it is **unmaintained**, provided as-is, and may not receive fixes for input or compatibility problems.
...
1. Download one of the four release editions: `jarock-cli-full`, `jarock-cli-lite`, `jarock-tui-full` or `jarock-tui-lite`. **CLI editions are strongly recommended and maintained.** TUI editions remain available with the compiled Windows terminal menu for `start-server.bat` and `parameter-manager.bat`, but the TUI is **unmaintained** and provided as-is. Full editions include Java installers; Lite editions do not. The selected edition is preserved by automatic updates. Then install a supported 64-bit Java 25 runtime for Minecraft 26.2. A direct installer link is available in the guide. **If you use the Eclipse Temurin installer:** during setup, enable the "Set JAVA_HOME variable" option (click the red X and select "Will be installed on local hard drive"). Without `JAVA_HOME`, Jarock may not find Java.
...
8. When the server finishes loading, Jarock prints the LAN connection addresses: Java uses the configured `server-port` over TCP, and Bedrock uses the configured Geyser UDP port. This address message remains visible even when `Show ready banner` is disabled; only the ASCII art is hidden. The multiplayer list uses the tracked `server/server-icon.png`; the server runtime also includes `server/icon.png`; new worlds receive the tracked root `icon.png` as their default world icon without replacing a custom world icon. All three tracked icons are included in every automatic CLI and TUI release ZIP. The original Jarock artwork is retained in the root and server icon assets. `clean-server-runtime.bat` preserves `server/icon.png` and `server/server-icon.png`; the root `icon.png` and `logo.png` are outside its cleanup scope. To remove only inactive updater downloads, rollback backups and temporary launcher wrappers, use `clean-cache.bat` (also available as `C. Clean Jarock cache` in the CLI parameter manager and `Clean .cache` in the TUI).
...
Startup update modes: `AUTO_UPDATE_MODE=install` checks for a compatible release and installs the verified package for the installed CLI/TUI and Full/Lite edition automatically; `AUTO_UPDATE_MODE=check` checks for updates only and never installs; `AUTO_UPDATE_MODE=never` does not check for updates and does not install updates. The default is `AUTO_UPDATE_MODE=install`; choose `check` or `never` in `parameter-manager.bat` to override it.
...
CLI editions are the strongly recommended and maintained path, using the original command-line flow. TUI editions keep the terminal menu open while DedicatedPower manages the server window; the menu returns after the server operation ends. TUI packages will continue to be published, but the TUI is unmaintained and receives no active maintenance or compatibility-fix guarantee. The guides explain installation from zero, Java and Bedrock networking, Geyser/Floodgate authentication, the LAN addresses printed after the ready banner, the verified Better Multiplayer Sleep datapack, optimization and redstone mods, world import and export, backups, security, troubleshooting and the limitations of client-side mods. The parameter manager edits a temporary copy and commits settings only through Save and exit or Save and start; Exit without saving leaves the existing settings unchanged. Press `U. Check for Jarock updates` to query GitHub without starting the server; if a verified compatible package for the installed edition is available, the updater asks `Download and install it now? (y/N)` before changing anything. It can also set Minecraft `online-mode`; keep it `true` by default, and use `false` only with a trusted authentication proxy because offline mode is unsafe on a public server. World import (`I`) asks `Remember this world for future starts? (Y/n)` after a source is selected; the default is Yes. A remembered source remains saved and is imported again at every start: Jarock asks for confirmation and first moves the existing world aside as `world_originalbkp`, so the imported world replaces the active one after the operator confirms (an incomplete world folder is replaced the same way). Answer `n` for a one-shot import. World export (`E`) mirrors the world to a destination folder outside `server/` after every clean shutdown. See [TODO.md](TODO.md) for the work that remains before public access.
