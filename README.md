...
To check for a newer Jarock release, enable `AUTO_UPDATE_CHECK` in `parameter-manager.bat`. On each startup Jarock checks GitHub when this option is enabled. If a newer compatible release is found, it asks `Download and install it now? (y/N)`: `y` installs the verified Lite package and `N` or Enter continues with the current version. It never updates silently. You can also stop the server and run `scripts/update-jarock.bat` manually; this explicit confirmation prevents a release from changing a working server unexpectedly. The updater reads the local version from `scripts/version.txt` (the numeric SemVer plus its channel suffix), checks the matching GitHub release channel, asks for confirmation, downloads the matching Lite release package (`jarock-lite.zip` for stable releases or `jarock-lite-<version>.zip` for beta releases), validates it together with its published SHA-512 checksum, and keeps a rollback backup. The Lite package is intentional: an existing installation already has its Java prerequisites, so updating does not download or reinstall them. It does not reinstall the existing loader or mods first, and it preserves the generated `server/` runtime, world, mods, libraries, local settings, Java selection, logs and secrets. It updates only the tracked project files included in the release package. Do not update while Minecraft is running and do not use `git pull` as a substitute for the updater.
